...
const replaceClose: (string: string, close: string, replace: string, index: number) => string = (string, close, replace, index) => {
...
void (async () => {
  ...
  isColorSupported =
    ...
    ('FORCE_COLOR' in env || argv.includes('--color') || process.platform === 'win32' || (isTTY && env.TERM !== 'dumb') || 'CI' in env)
